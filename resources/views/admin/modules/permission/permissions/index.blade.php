@extends('master.admin.layout.app', ['title' => 'Permissions'])
...
@section('content')
    ...
    {{-- Modal Action --}}
    @include('admin.modules.permission.permissions.modal.action')
    {{-- End Modal Action --}}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        let dataTable = $("#datatable").DataTable({
            ...tableOptions,
            ajax: "{{ route('admin.permissions.index') }}?type=datatable",
            processing: true,
            serverSide : true,
            responsive: false,
            destroy: true,
        ...
        //----form environtment
        let ajaxUrl = "{{ route('admin.permissions.store') }}";
        let ajaxType = "POST";

        function clearForm() {
            $("#permission-form").find('input').val("");
            $('#permission-form').find('.error').text("");

            ajaxUrl = "{{ route('admin.permissions.store') }}";
            ajaxType = "POST";
        }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status == 404) {
                        ...
                        ajaxUrl = "{{ route('admin.permissions.index') }}/"+response.data.id;
                        ajaxType = "PUT";
