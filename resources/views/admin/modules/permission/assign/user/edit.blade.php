        ...
        <div class="row">
            <div class="col-sm-7 col-auto">
                ...
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">Permission To User</li>
                ...
                <div class="card">
                    <div class="card-body">
                        @if(session()->has('error'))
                            ...
                            @endphp
                        @endif
                        <form action="{{ route('admin.assign.user.update', Crypt::encryptString($data->id)) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row form-row">
                                <div class="col-12 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select name="user" class="select @error('user') is-invalid @enderror">
                                            <option selected disabled>-- Select User --</option>
                                            @foreach ($users as $item)
                                            <option value="{{ $item->id }}"
                                                {{ old('user', $data->id) == $item->id ? 'selected' : null }}>{{ $item->name }}
                                            </option>
                                            @endforeach
                                        </select>
                                        @error('user')
                                        ...
                                        <div class="checkbox">
                                            ...
                                            <label class="mr-3 mb-3">
                                                <input type="checkbox" class="@error('roles') is-invalid @enderror" name="roles[]" 
                                                    value="{{ $item->name }}" {{ $data->roles()->find($item->id) ? "checked" : "" }} > {{ $item->name }}
                                            </label>
                                            @endforeach
                                        </div>
                                        @error('roles')
                            ...
                            <div class="text-right">
                                ...
                                <a href="{{ route('admin.assign.user.index') }}" class="btn btn-danger">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
