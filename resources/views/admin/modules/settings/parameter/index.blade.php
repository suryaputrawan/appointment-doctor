@extends('master.admin.layout.app', ['title' => 'Setting Parameter'])
...
@section('content')
    ...
    {{-- Modal Action --}}
    @include('admin.modules.settings.parameter.modal.action')
    {{-- End Modal Action --}}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        let dataTable = $("#datatable").DataTable({
            ...tableOptions,
            ajax: "{{ route('admin.setting-params.index') }}?type=datatable",
            processing: true,
            serverSide : true,
            responsive: false,
            destroy: true,
            columns: [
                ...
                { data: "format_surat", name: "format_surat", orderable: true  },
        ...
        //----form environtment
        let ajaxUrl = "{{ route('admin.setting-params.store') }}";
        let ajaxType = "POST";

        function clearForm() {
            $("#param-form").find('input').val("");
            $('#param-form').find('.error').text("");
            $("#hospital").val("").trigger('change');

            ajaxUrl = "{{ route('admin.setting-params.store') }}";
            ajaxType = "POST";
        }
        ...
        $('#param-form').on('submit', function(e) {
            ...
            $('#letter-type-form').find('.error').text("");
            ...
            $.ajax({
                ...
                success: function(response) {                   
                    ...
                    if (response.status == 200) {
                        ...
                        $.each(response.errors.format_surat, function(key, error) {
                            $('#error-format_surat').append(error);
                        });
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        ajaxUrl = "{{ route('admin.setting-params.index') }}/"+response.data.id;
                        ajaxType = "PUT";

                        $('#format_surat').val(response.data.format_surat);
